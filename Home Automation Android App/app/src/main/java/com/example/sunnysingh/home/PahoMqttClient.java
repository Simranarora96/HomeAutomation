...
public class PahoMqttClient {
    ...
    public void publishMessage(@NonNull MqttAndroidClient client, @NonNull JSONObject msg, @NonNull String topic)
            throws MqttException, UnsupportedEncodingException {
        byte[] encodedPayload = new byte[0];

        encodedPayload = msg.toString().getBytes("UTF-8");
        MqttMessage message = new MqttMessage(encodedPayload);
        message.setId(320);
        message.setRetained(true);
        //message.setQos(qos);
        client.publish(topic, message);
        Log.d("published",message.toString());
    }
